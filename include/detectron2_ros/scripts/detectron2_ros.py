...
class Detectron2node(object):
    def __init__(self):
        ...
        self._gen_image_from_pcd = rospy.get_param("~gen_image_from_pcd")
        self._use_custom_model = rospy.get_param("~use_custom_model")
        self._enable_quater_pcd = rospy.get_param("~enable_quater_pcd")
        ...
        # Mask R-CNN settings
        self.cfg = get_cfg()
        self.cfg.merge_from_file(rospy.get_param("~model_cfg"))
        ...
        if self._use_custom_model:
            self.cfg.MODEL.WEIGHTS = rospy.get_param("~custom_weight")
        else:
            self.cfg.MODEL.WEIGHTS = rospy.get_param("~base_weight")
        ...
        if self._gen_image_from_pcd:
            ...
            if self._use_compressed_image:
                self._image_sub = rospy.Subscriber("/input_image", CompressedImage, self._image_callback, queue_size=1)
            else:
    ...
    def convert_to_cv_image(self, image_msg):
        ...
        # if use normal image
            ...
            elif image_msg.encoding.lower() == 'bgr8':
...
def main(argv):
    ...
    rospy.loginfo("Initialize detectron2")
    node.run()
